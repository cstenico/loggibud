...
from loggibud.v2.types import VRPPDSolution, Point
from loggibud.v2.distances import OSRMConfig
...
def plot_vrpdp_solution_routes(
    ...
    for i, vehicle in enumerate(vehicles_subset):
        ...
        for demand in vehicle.demands:
            folium.Circle(
                location=(demand.point.lat, demand.point.lng),
                radius=10,
                fill=True,
                color=vehicle_color,
                popup=(
                    f"Vehicle {i} ({demand.point.lat}, {demand.point.lng})"
                ),
            ).add_to(m)
...
def plot_vrpdp_solution(
    solution: VRPPDSolution, route_indices_to_plot: Optional[List[int]] = None
) -> None:
